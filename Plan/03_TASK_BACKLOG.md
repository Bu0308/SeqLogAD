# 03 — Task Backlog v1.1 (Historical)

> The forward v2 backlog is defined in [`master-implementation-plan-v2-domain-adaptive-fusion.md`](master-implementation-plan-v2-domain-adaptive-fusion.md) and the v2 workbook. This file preserves completed v1.1 task provenance.

Statuses: `NOT_STARTED`, `IN_PROGRESS`, `HUMAN_DECISION_REQUIRED`, `FROZEN_HUMAN_REVIEW_READY`, `FROZEN_HUMAN_APPROVED`, `AUTHORIZED_NEXT_TASK`, `BLOCKED_PENDING_HUMAN_APPROVAL`, `BLOCKED`, `DONE`, `CONDITIONAL`, `FUTURE`, `SUPERSEDED`. Owners: `AI`, `HUMAN`, `AI_PREP/HUMAN_EXECUTE`.
...
| PURGE-DECISION-001 | MUST | HUMAN | PURGE-AUDIT-001 | FROZEN_HUMAN_APPROVED | Option B is frozen before scientific outcomes: primary split/parser/TEST/EFFECT identities unchanged; secondary whole-component purge sensitivity is pre-registered, non-selection, and `NOT_RUN`; canonical events are authorized next |
| CANONICAL-NUL-DECISION-001 | MUST | HUMAN | PARSE-001, PURGE-DECISION-001 | FROZEN_HUMAN_REVIEW_READY | Human either approves or rejects the versioned collision-safe NUL representation proposal; no real canonical corpus is generated and all split/parser/TEST identities remain frozen |
...
| CANONICAL-EVENT-001 | MUST | AI | PARSE-001, ALIGN-FIX-001, PURGE-DECISION-001, CANONICAL-NUL-DECISION-001=FROZEN_HUMAN_APPROVED | BLOCKED_PENDING_HUMAN_APPROVAL | After explicit human approval, frozen HDFS/BGL parsers emit partition-scoped canonical events with strict no-update semantics, stable identities, label isolation, one-line-one-event preservation, and no TEST access |
...
| FINAL-TEST-001 | MUST | HUMAN | FINAL-FREEZE-001, SPLIT-001 | NOT_STARTED | TEST opened once through the human-only seal workflow; immutable result bundle produced |
