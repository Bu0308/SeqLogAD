...
## ADR-029 — Exact raw split semantics and non-circular split identity

- **Date:** 2026-08-22
- **Status:** ACCEPTED — HUMAN APPROVED.
- **Context:** SPLIT-001 stopped before generation because Protocol v1.1 did not uniquely fix its allocation denominator, rounding, connected-component boundary behavior, BGL residual scope, or split/assignment/partition/file hash layering. No real split or TEST partition existed.
- **Evidence:** The targeted 2024–2026 review supports HDFS block/session atomicity, BGL chronological raw split-before-window construction, and fixed non-overlapping windows, while exposing conflicting ratios/grouping/label policies. It does not establish SeqLogAD's exact five-way floor, connected-component purge, residual accounting, hash design, or physical seal. See `docs/literature/split-protocol-evidence-matrix.md` and `docs/references/PROTOCOL-SPLIT-CLARIFY-001-citations.md`.
- **Decision:** Freeze PROTOCOL-SPLIT-CLARIFY-001 v1.0 as a binding Protocol-v1.1 addendum. Use cumulative-floor `60/10/10/10/10` boundaries without labels or repair. HDFS ranks structurally eligible lines, assigns whole META-001 components only when all ranks occupy one nominal partition, otherwise purges the whole component. BGL allocates raw source ranks first, then creates independent non-overlapping 100-line parents per partition and explicitly excludes each 1–99-line tail. Define canonical split payload hash, post-payload assignment IDs, structural-membership partition hashes, and separate exact-file hash.
...
## ADR-034 — Preserve HDFS primary split and pre-register secondary purge sensitivity

- **Date:** 2026-08-24
- **Status:** `ACCEPTED / FROZEN_HUMAN_APPROVED`.
- **Context:** PURGE-AUDIT-001 found a substantial non-random structural exclusion and a modest but detectable aggregate prevalence difference before any baseline/model/KT/TEST result. Dataset, split, parser, EFFECT-001, and both TEST identities still verified exactly; TEST had zero opens and zero unlock records.
- **Evidence classification:** Recent log-anomaly and event-sequence literature supports treating grouping, chronology, parser, and dataset construction as consequential (`LITERATURE_SUPPORTED`). Recent methods guidance informs pre-specified sensitivity analysis before outcome inference, with explicit non-ML domain limitations (`LITERATURE_INFORMED_SEQLOGAD_DECISION`). Retaining this exact primary and defining this exact purge cohort are `SEQLOGAD_PROTOCOL_DECISION`; canonical hashing and artifact separation are `ENGINEERING_DECISION`.
- **Decision:** Select Option B. Keep the original retained-component SPLIT-001 HDFS population as the sole primary analysis. Pre-register `HDFS-PURGE-SENSITIVITY-001` over all complete public `PURGED_BOUNDARY` components as `SECONDARY / ROBUSTNESS_ONLY / NON-CONFIRMATORY / NON-SELECTION / NON-TUNING / NOT_RUN`. Reuse the frozen parser and, after the primary HDFS result bundle is immutable, reuse the frozen primary methods/configs/seeds/AP estimand and paired component uncertainty. Never use the sensitivity outcome to choose models, gates, thresholds, or the primary claim.
- **Alternatives:** Option A (limitation only) leaves robustness unmeasured. Option C (replace primary) would invalidate split/TEST/parser provenance without evidence that the current primary is scientifically indefensible.
- **Interpretation:** Matching non-inconclusive EFFECT-001 regions supports robustness. Incompatible non-inconclusive regions require `HDFS_RESULT_SENSITIVE_TO_SPLIT_CONSTRUCTION`; an inconclusive/unclassifiable sensitivity requires an explicit limitation. The original primary remains controlling.
- **Integrity:** `delta_HDFS = delta_BGL = 0.01 AP`, split payload, TEST hash, parser state, and one-time human TEST policy remain unchanged. Purge prevalence and Delta AP are different estimands and are not compared.
- **Consequence:** The human stop gate is closed. `CANONICAL-EVENT-001` is the one authorized next task. No sensitivity run, canonical event, sequence, baseline, model, metric, TEST grant, commit, or push was produced by this decision.
- **Artifacts:** `docs/decisions/PURGE-DECISION-001.md`, `configs/protocols/purge-decision-v1.yaml`, and `docs/references/PURGE-DECISION-001-citations.md`; canonical decision payload SHA-256 `5af8505364c793b2fbd42885ebcdea1eba03b75c808415214af7311aa4ecd177`.

## ADR-035 — Adopt v2 Domain-Adaptive Fusion as the forward research plan

- **Date:** 2026-08-28
- **Status:** ACCEPTED — USER APPROVED / PLANNED, EMPIRICAL STATUS `NOT_RUN`.
- **Context:** The user redirected the forward question from v1.1 single-benchmark sequence-added-value measurement to zero-label cross-system transfer and target-normal adaptation. The repository already contained a v2 plan/workbook/DOCX, but README/docs/config entry points still called v1.1 active.
- **Decision:** Make `DOMAIN-ADAPTIVE-FUSION-001` and `Plan/master-implementation-plan-v2-domain-adaptive-fusion.md` the forward source-of-truth. Fix the stack to Llama-3.1-8B with independent `LoRA-Semantic` and `LoRA-Sequence` adapters plus GTAT; require leave-one-architecture-out chronology, unlabeled target-buffer calibration, equal-weight comparison and G0–G4 gates.
- **Superseded:** v1.1 sequence-added-value RQs, KT-1–KT-3 ladder, conditional Transformer/localization/F0–F1 fusion and HDFS/BGL-as-final-target framing are no longer forward objectives. V1/V2/V3 product/four-expert plans remain historical.
- **Preserved:** immutable data/manifests/checksums, label isolation, chronological discipline, deterministic identities, physical TEST seals, normal-only frozen Drain3 state, EFFECT-001 and split/purge decisions remain historical foundation provenance and are not rewritten.
- **Consequence:** This decision changes documentation/state authority only. No model, target fold, adaptation, metric, checkpoint or TEST result is created. The next v2 work is P1.1; downstream data-collection guidance cannot change this scientific contract.
- **Evidence:** `Plan/master-implementation-plan-v2-domain-adaptive-fusion.md`, `docs/research-protocol-v2-domain-adaptive-fusion.md`, `docs/plan-migration-v1.1-to-v2.md`, and `configs/protocols/protocol-v2-domain-adaptive-fusion.yaml`.
